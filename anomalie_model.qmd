...
<!-- ```{r} -->
<!-- wynikiJH <- readxl::read_excel("Model_JH_a_prokuratura.xlsx", skip = 2) -->
<!-- wynikiJH <- wynikiJH %>%  -->
<!--   select(Gmina, `Numer komisji`, `OGÓŁEM...5`, `Rafał Trzaskowski...7`, -->
<!--     `Niedomiar Trzaskowskiego wedle modelu`, `Rafał Trzaskowski...14`) %>%  -->
<!--   set_names(c("Gmina", "Komisja", "N", "Trzask", "Model", "Wynik")) %>%  -->
<!--   drop_na(Komisja) %>%  -->
<!--   mutate(Wynik = ifelse(is.na(Wynik), 0, Wynik)) %>%  -->
<!--   mutate(Trzask_podane = Trzask/N*100, Trzask_prawdziwe = (Trzask + Wynik)/N*100, -->
<!--     Trzask_JH = (Trzask + Model)/N*100) -->

<!-- wynikiJH %>%  -->
<!--   filter(! Gmina %in% c("m. Ostrów Mazowiecka", "gm. Łęczna")) %>%  -->
<!--   mutate(Różnica = Trzask_prawdziwe - Trzask_JH) %>%  -->
<!--   summarise(mean(Model - Wynik), mean(abs(Model - Wynik)), -->
<!--     mean(Trzask_JH - Trzask_prawdziwe), mean(abs(Trzask_JH - Trzask_prawdziwe))) -->

<!-- # wyb25proc %>% filter(Gmina == "gm. Choczewo") %>% view() -->
<!-- # wyb20proc %>% filter(Gmina == "gm. Choczewo") %>% view() -->
<!-- ``` -->

<!-- Mój model. Chcę porównać się ze wszystkimi zbadamy komisjami, więc buduję model tak, jak budowałem, ale następnie stosuję na wszystkich danych. -->

<!-- ```{r} -->
<!-- wyniki <- readxl::read_excel("data/Tabela_250_okw.xlsx", skip = 2) -->
<!-- wyniki <- wyniki %>%  -->
<!--   select(Gmina, `Numer komisji`, `Siedziba komisji`, `OGÓŁEM...5`, `OGÓŁEM...8`, -->
<!--     `Rafał Trzaskowski...7`, `Rafał Trzaskowski...10`) %>%  -->
<!--   set_names(c("Gmina", "Komisja", "Siedziba", "N_podane", "N_prawdziwe", -->
<!--     "Trzask_podane", "Trzask_prawdziwe")) %>%  -->
<!--   drop_na(Komisja) %>%  -->
...
<!-- # zamian wszystkich głosów o tylko 17, zamiana kupki głosów 48, -->
<!-- # w 1 przypadku nie zaliczono 20 głosów na Trzask, dla Nawr było ok -->
...
<!-- Wskazałem 97, ale zbadanych zostało 75, Z tego w 46 rzeczywiście były nieprawidłowości, 11 były ok, a 17 przeoczyłem (bh.p na granicy poza 1 przypadkiem). -->

<!-- Czy da się oszacować, ile było błędów we wszystkich komisjach? Najlepiej zrobić to na podstawie modelu JH, tak że w pierwszej kolejności muszę go odtworzyć. -->
...
<!--   mutate(Błąd = ifelse(Różnica >= 5, 1, 0)) %>%  -->
...
<!-- # nieprawidłowości w 204 komisjach? (0.7%) -->

<!-- ggplot(anomalie, aes(Reszta_stand, Różnica_pp)) + -->
<!--   geom_point(alpha = 0.6, size = 2) + -->
<!--   geom_smooth(method = "lm", fullrange = TRUE, -->
<!--     data = anomalie %>% filter(Różnica > 5)) + -->
<!--   scale_x_reverse(limits = c(NA, 0)) + -->
<!--   geom_hline(yintercept = 0, linetype = 2) + -->
<!--   labs(y = "Rzeczywista różnica głosów [p.p.]", x = "Standaryzowany błąd przewidywania") + -->
<!--   theme( -->
<!--     axis.line = element_blank(), -->
<!--     axis.ticks = element_blank(), -->
<!--     panel.grid.major = element_line(color = "gray90", linewidth = 0.5) -->
<!--   ) -->
<!-- lm(Różnica_pp ~ Reszta_stand, data = anomalie %>% filter(Różnica > 5)) -->
<!-- # -1.949 + 2.481x = 0 -->
<!-- # x = 1.949/2.481 = 0.786 -->
...
<!--   scale_x_reverse(limits = c(NA, 0)) + -->
<!--   labs(y = "Skumulowana liczba komisji z wykrytymi\nnieprawidłowościami", -->
<!--     x = "Standaryzowany błąd przewidywania") + -->
<!--   theme( -->
<!--     axis.line = element_blank(), -->
<!--     axis.ticks = element_blank(), -->
<!--     panel.grid.major = element_line(color = "gray90", linewidth = 0.5) -->
